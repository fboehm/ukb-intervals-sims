            ...
            for p in `seq 1 10`; do
                for fold in `seq 1 5`; do
                    ...
                    if [ ${k} -eq ${SLURM_ARRAY_TASK_ID} ]; then
                        ...
                        #DBSLMMpath=/net/mulan/disk2/yasheng/predictionProject/code/ # path to dbslmm C++ compiled
                        DBSLMMpath=/net/mulan/disk2/fredboe/research/
                        blockf=/net/mulan/disk2/fredboe/research/ukb-intervals/LDblock_EUR/chr
                        #ref=${compstr}04_reference/ukb/geno/chr
                        ref=${fbStr}dat-quant-136k/reference/chr # use these same ref subjects

                        # genotypes (ie, plink files) for validation set
                        val=~/research/ukb-intervals-sims/dat-quant-136k/validation/chr # use same validation set, too.

                        if [[ "$dat" == "continuous" ]]; then
                            phenoVal=~/research/ukb-intervals-sims/dat-quant-small-scale-simulations/validation/scenario${scenario}_${distribution}_hsq${hsq}_replicate${p}.txt
                            index=r2
                        else
                        ...
                        if [[ "$dat" == "continuous" ]]; then
                            herit=${hsq}
                            summ=~/research/ukb-intervals-sims/dat-quant-small-scale-simulations/gemma/scenario${scenario}/${distribution}/hsq${hsq}/output/summary_ukb_pheno${p}_scenario${scenario}_${distribution}_hsq${hsq}_fold${fold}_chr
                            outPath=~/research/ukb-intervals-sims/dat-quant-small-scale-simulations/DBSLMM/scenario${scenario}/${distribution}/hsq${hsq}/
                        else
