...
idxSub=~/research/ukb-intervals-sims/dat/hsq${hsq}_pcausal${pcausal}/verification-ids.txt
#for p in `seq 1 10`; do
  for chr in `seq 1 22`; do
    let k=${k}+1
    if [ ${k} -eq ${SLURM_ARRAY_TASK_ID} ]; then
      bfileAll=/net/mulan/disk2/yasheng/predictionProject/plink_file/ukb/chr${chr}
      bfileSub=~/research/ukb-intervals-sims/dat/hsq${hsq}_pcausal${pcausal}/verification/allchr${chr}
      bfileSubP=~/research/ukb-intervals-sims/dat/hsq${hsq}_pcausal${pcausal}/verification/chr${chr}
