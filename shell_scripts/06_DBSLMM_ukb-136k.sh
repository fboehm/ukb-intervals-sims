...
ref=${fbStr}dat-quant-136k/reference/chr

# genotypes (ie, plink files) for validation set
val=~/research/ukb-intervals-sims/dat-quant-136k/validation/chr

if [[ "$dat" == "continuous" ]]
then
phenoVal=~/research/ukb-intervals-sims/dat-quant-136k/validation/scenario${scenario}_${distribution}_hsq${hsq}_replicate${p}.txt
index=r2
else
...
summ=~/research/ukb-intervals-sims/dat-quant-136k/gemma/scenario${scenario}/${distribution}/hsq${hsq}/output/summary_ukb_pheno${p}_scenario${scenario}_${distribution}_hsq${hsq}_fold${fold}_chr
#outPath=/net/mulan/disk2/yasheng/comparisonProject/05_internal_c/pheno${p}/DBSLMM/
outPath=~/research/ukb-intervals-sims/dat-quant-136k/DBSLMM/scenario${scenario}/${distribution}/hsq${hsq}/
else
