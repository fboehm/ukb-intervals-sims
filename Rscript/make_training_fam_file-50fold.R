...
for (fold in 1:n_folds){
  test_ids[[fold]] <- vroom::vroom(file = paste0("../hapmap3-50fold/test-ids-fold", fold,".txt" ), col_names = FALSE)
  training_ids[[fold]] <- vroom::vroom(file = paste0("../hapmap3-50fold/training-ids-fold", fold,".txt" ), col_names = FALSE)
}
...
for (scenario in scenario_vec){
  for (distribution in distribution_vec){
    for (hsq in hsq_vec){
      ...
      outdir <- paste0("../dat-quant-50fold/gemma/scenario", scenario, "/", distribution, "/hsq", hsq)
