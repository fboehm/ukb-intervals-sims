...
library(magrittr)
ids <- vroom::vroom(file = "../hapmap3-small-scale-simulations/plink_files_for_sims/chr1.fam", col_names = FALSE)
...
training_ids <- list()
for (fold in 1:5){
  test_ids[[fold]] <- vroom::vroom(file = paste0("../hapmap3-small-scale-simulations/test-ids-fold", fold,".txt" ), col_names = FALSE)
  training_ids[[fold]] <- vroom::vroom(file = paste0("../hapmap3-small-scale-simulations/training-ids-fold", fold,".txt" ), col_names = FALSE)
}
...
for (scenario in scenario_vec){
  for (distribution in distribution_vec){
    for (hsq in hsq_vec){
      #trait_file <- paste0("../hapmap3-small-scale-simulations/sim_traits/sims_scenario", scenario, "_", distribution, "_hsq", hsq, ".txt.phen")
      ...
      outdir <- paste0("../dat-quant-small-scale-simulations/gemma/scenario", scenario, "/", distribution, "/hsq", hsq)
