...
for (scenario in scenario_vec){
  for (distribution in distribution_vec){
    for (hsq in hsq_vec){
      ...
      (resfn <- paste0("../dat-quant-136k/DBSLMM/scenario", 
                       scenario, "/", 
                       distribution, "/hsq", 
                       hsq, "/replicate", phe_num, "_res-136k.rds"))
      cat(  
      knitr::knit_child(
        'cv-plus-simulations-child-136k.Rmd', 
        envir = environment(), 
        quiet = TRUE
      )
      )
...
saveRDS(object = res_list, file = "res_list-136k.rds")
```


```{r}
res_list <- readRDS("res_list-136k.rds")
...
```{r}
plots$plots[[1]]
ggsave(filename = "../results/quant_scenarioI-136k.jpg")
```

```{r}
plots$plots[[2]]
ggsave(filename = "../results/quant_scenarioII-136k.jpg")
```

```{r}
plots$plots[[3]]
ggsave(filename = "../results/quant_scenarioIII-136k.jpg")
```


```{r}
plots$plots[[4]]
ggsave(filename = "../results/quant_scenarioIV-136k.jpg")
```

## Interval widths by trait
