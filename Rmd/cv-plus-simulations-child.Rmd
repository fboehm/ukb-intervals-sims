...
phen_file <- here::here("hapmap3", "sim_traits", paste0("sims_scenario", scenario, "_", distribution, "_hsq", hsq, ".txt.phen"))
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:1){
    ...
    profile_fn <- here::here("dat-quant", "DBSLMM", paste0("scenario", scenario), distribution, paste0("hsq", hsq), pfn)
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:1){
    ...
    fn <- here::here("dat-quant", "verification", "allele-scores", paste0("scenario", scenario), distribution, paste0("hsq", hsq), paste0("pred_replicate", phe_num, "_fold", cross_num, "_chr", chr_num, "_best.dbslmm.txt.profile"))
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:1){
    s2[[chr_num]] <- out[[cross_num]][[chr_num]] %>%
      dplyr::select(1,2,6)  # choose only the SCORESUM column
...
for (i in 1:n_verification){
  ...
  intervals[[i]]<- c(quantile(o3$fitted_minus_residual, 
                              prob = floor((n_training + 1) * (alpha / 2)) / n_training, 
                              type = 6), 
  quantile(o3$fitted_plus_residual, 
           prob = ceiling((n_training + 1) * (1 - alpha / 2)) / n_training, 
           type = 6)
  )
}
```
