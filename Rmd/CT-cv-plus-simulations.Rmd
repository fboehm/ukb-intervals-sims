...
res_list <- list()
for (scenario in scenario_vec[1]){
  for (distribution in distribution_vec){
    for (hsq in hsq_vec){
      for (phe_num in 1:n_replicates){
      (resfn <- paste0("../dat-quant/CT/scenario", 
                       scenario, "/", 
                       distribution, "/hsq", 
                       hsq, "/replicate", phe_num, "_res.rds"))
      cat(  
      knitr::knit_child(
        'CT-cv-plus-simulations-child.Rmd', 
        envir = environment(), 
        quiet = TRUE
      )
      )
...
saveRDS(object = res_list, file = "CT-res_list.rds")
```


```{r}
res_list <- readRDS("CT-res_list.rds")
# set up a tibble with the correct ordering of combinations of scenario, distribution, hsq, and replicate number
coverages <- numeric()
for (i in 1:90){ # loop over all traits
  coverages[i] <- res_list[[i]] %>%
      dplyr::summarise(coverage = mean(in_interval))
}
results <- expand.grid(1:10, hsq_vec, distribution_vec, scenario_vec[1], stringsAsFactors = FALSE) %>%
  tibble::as_tibble() %>%
...
plots$plots[[1]]
ggsave(filename = "../results/CT-quant_scenarioI.jpg")
```
