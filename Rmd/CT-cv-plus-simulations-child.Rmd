...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:1){
    ...
    profile_fn <- here::here("dat-quant", "CT", paste0("scenario", scenario), distribution, paste0("hsq", hsq), pfn)
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:1){
    ...
    fn <- here::here("dat-quant", "verification", "CT-allele-scores", paste0("scenario", scenario), distribution, paste0("hsq", hsq), paste0("pred_replicate", phe_num, "_fold", cross_num, ".txt.profile"))
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:1){
    s2[[chr_num]] <- out[[cross_num]][[chr_num]] %>%
      dplyr::select(1,2,6)  # choose only the SCORESUM column & ID COLUMNS
